...
namespace DiscordSCLDbot215.Services
{
    ...
    public static class LoggingService
    {
        ...
        public static async Task LogAsync(string src, LogSeverity severity, string message, Exception exception = null)
        {
            ...
            string text_write = "";
            text_write += "[" + DateTime.Now.ToString("HH:mm:ss") + "] ";

            await Append(text_write , ConsoleColor.White);
            await Append($"{GetSeverityString(severity)}", GetConsoleColor(severity));
            await Append($" [{SourceToString(src)}] ", ConsoleColor.DarkGray);
            text_write += GetSeverityString(severity) + $" [{SourceToString(src)}] ";
            ...
            else if (exception == null)
            {
                ...
                text_write += "Uknown Exception. Exception Returned Null.";
            }
            else if (exception.Message == null)
            {
                ...
                text_write += $"{exception.Message ?? "Unknownk"}\n{exception.StackTrace ?? "Unknown"}";
            }

            new SCLD_tools().write_log_file(text_write);
        }
